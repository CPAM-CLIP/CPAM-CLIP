...
class ModifiedResNet(nn.Module):
    ...
    def forward(self, x, return_all=False, csa=True):
        def stem(x):
        ...
        x = self.layer3(x)
        x = self.layer4(x)
      #  x = self.attnpool(x, return_all)
        
        b, c ,h ,w = x.shape
        x = x.reshape(b, c, h * w).permute(2, 0, 1)
        x = F.linear(x, self.attnpool.v_proj.weight, self.attnpool.v_proj.bias)
        x = F.linear(x, self.attnpool.c_proj.weight, self.attnpool.c_proj.bias)
      # x = x.squeeze()
      #  print("xx",x.shape)
        return x
...
class VisionTransformer(nn.Module):
    def __init__(self, input_resolution: int, patch_size: int, width: int, layers: int, heads: int, output_dim: int):
        ...
        # for name, param in self.transformer.resblocks[-1:][0].named_parameters():
        #     print(name, param)
        
        # #print(self.transformer.resblocks[-1:].attn)
        # for name, param in self.transformer.resblocks[-1:][0].attn.named_parameters():
        #     print(name, param)
        #     if 'out_proj.weight' in name:
        #         # out_proj.weight가 value를 transformation하는 weight입니다.
        #         print(name, param)
        
        self.window = 0
        self.slide = 0
        self.intermediate_feat_before_last = 0
        self.intermediate_feat_after_last = 0
        
        self.version = 0
    def forward(self, x: torch.Tensor, window, slide, return_all=False, csa=True):
        img  = x[0]
        B, nc, w, h = x.shape
...
#                 x = blk(x)
    ...
    def custom_attn(self, attn_layer, x, img , return_attn=False, with_attn=False, csa=False):
